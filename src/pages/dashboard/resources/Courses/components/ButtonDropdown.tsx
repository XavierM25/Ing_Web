...
const CustomDropdown = ({ options, onSelectionChange }: Props) => {
  ...
  const renderOptionControl = (option: Option, isSelected: boolean) => {
    if (option.category === 'Precio') {
      return (
        <div className={`w-4 h-4 border rounded-full transition-all duration-150 flex items-center justify-center ${
          isSelected
            ? 'border-blue-500'
            : 'border-gray-300'
        }`}>
              ...
              {items.map((option) => (
                ...
                >
                  <div className="flex items-center space-x-2">
                    {renderOptionControl(option, selectedOptions.has(option.id))}
